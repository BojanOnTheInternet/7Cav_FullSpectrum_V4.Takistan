...
OO_TRACE_DECL(HALO_Start) =
{
	...
	[_vehicle, _match select 1] spawn
	{
		...
		([_vehicle] call _jumpCode) params ["_targetDrop", "_targetPosition", "_targetDelay"];
		...
		while { ([player, _vehicle] call HALO_ReadyForJump) } do
		{
			...
			private _currentDrop = _targetDrop;
			...
			while { ([player, _vehicle] call HALO_ReadyForJump) && _remainingDelay > 0 && { _targetDrop == _currentDrop } && { _targetDrop != "" } } do
			{
				...
				{
					_remainingDelay = floor _remainingDelay;
					[format ["<t align='center' size='2'>%1</t>", _remainingDelay], -1, -1, 0.0, 0.0] call BIS_fnc_dynamicText;
				...
				else
				{
					...
					sleep 1;
					_remainingDelay = _jumpTime - ([] call HALO_CurrentTime);
				};
